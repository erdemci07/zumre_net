...
class _StudyGuardHomeScreenState extends State<StudyGuardHomeScreen> {
  ...
  bool _isInstitutionBlockingStudy = false;
  ...
  String? _institutionStudyBlockMessage(Map<String, dynamic>? data) {
    if (data == null) return null;

    final mode = '${data['institutionMode'] ?? 'active'}';
    if (mode == 'closed' && data['closedDate'] == _todayDateKey()) {
      return 'Kurum bugün kapalıdır. Yeni etüt başlatılamaz.';
    }
    ...
    if (mode == 'exam' &&
      ...
      return 'Deneme modu aktif. Etüt işlemleri geçici olarak durduruldu.';
    }
  ...
  Future<bool> _applyLocalStudyScheduleFromCache({
    ...
    if (_isInstitutionBlockingStudy) return true;
  ...
  Future<bool> _applyRuntimeStudyState(Map<String, dynamic>? data) async {
    final institutionMessage = _institutionStudyBlockMessage(data);
    if (institutionMessage != null) {
      if (!mounted) return true;
      setState(() {
        _isInstitutionBlockingStudy = true;
        _isStudyOpenNow = false;
        _activeStudySlotText = 'Etüt saati';
        _studyScheduleMessage = institutionMessage;
      });
      return true;
    }
  ...
  Future<void> _finishStudySessionSilently({
    ...
    try {
      ...
      final studentsSnapshot = await sessionRef.collection('students').get();
  ...
  Future<bool> _addStudentToStudy(DocumentSnapshot doc) async {
    ...
    final sessionRef = _firestore.collection('studySessions').doc(sessionId);
    final sessionStudentRef = sessionRef.collection('students').doc(doc.id);
    ...
    try {
      ...
      _showSnack(
        wasRejoin ? 'Öğrenci yeniden etüte alındı.' : 'Öğrenci etüte alındı.',
      );
      return true;
  ...
  Widget _sessionCard() {
    final active = _isStudyOpenNow && _activeSessionId != null;
                      ...
                      child: StreamBuilder<QuerySnapshot>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          final teachers = snapshot.data!.docs.where((doc) {
                            ...
                            final isSelected = tempTeacherId != null &&
                                doc.id == tempTeacherId;
  ...
  Widget _currentStudents() {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final snapshotStudentIds =
            snapshotStudents.map((doc) => doc.id).toSet();
        ...
        final totalStudentCount =
            optimisticStudents.length + snapshotStudents.length;
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final id = isOptimistic
                        ...
                        : snapshotStudents[index - optimisticStudents.length]
                            .id;
                    ...
                    return Container(
                      ...
                      child: ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed:
                              isOptimistic || _removingStudentIds.contains(id)
                                  ? null
